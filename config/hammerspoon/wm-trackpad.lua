...
function get_window_under_mouse()
   ...
   return hs.fnutils.find(hs.window.orderedWindows(),
      function(w) return my_screen == w:screen()
         and w:isStandard()
         and (not w:isFullScreen())
         and my_pos:inside(w:frame())
      end)
end

dragging = {} -- global variable to hold the dragging/resizing state

drag_event = hs.eventtap.new({ hs.eventtap.event.types.mouseMoved },
   function(e)
...
flags_event = hs.eventtap.new({ hs.eventtap.event.types.flagsChanged },
   function(e)
      ...
      local mode = (
         flags.shift and 1 or 0)
         + (flags.cmd and 2 or 0)
         + (flags.alt and 4 or 0)
