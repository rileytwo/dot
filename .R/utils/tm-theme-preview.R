...
GeomDotplot <- proto(
  Geom, {
    ...
    new <- function(.,
      ...
      # or position is position_stack() (the test
      # is kind of complex), tell them to
      # use stackgroups=TRUE instead. Need to
      # use identical() instead of ==, because == will fail if object is
      # position_stack() or position_dodge()

      if (!is.null(position) &&
        (identical(position, "stack") ||
          (is.proto(position) && position$objname == "stack"
          )
        )
      ) {
        message(
          "position=\"stack\" doesn't work
                            properly with geom_dotplot.
        ...
        if (
          ...
          message(
            'geom_dotplot called with stackgroups=TRUE
            and method="dotdensity".
            You probably want to set binpositions="all"'
          )
          do.call(
            "layer",
            list(
    ...
    reparameterise <- function(., df, params) {
      ...
      # If stackgroups=TRUE, split only on x (or y)
      # and panel; if not stacking, also split by group
      plyvars <- params$binaxis %||% "x"
      plyvars <- c(plyvars, "PANEL")

      if (is.null(params$stackgroups) ||
        !params$stackgroups
      ) {
      ...
      if (is.null(params$binaxis) ||
        ...
        # After position code is rewritten, each dot should have its own bbox.
        ...
        # the bounding rectangle for each stack

        # Can't do bounding box per dot, because x position isn't real.
        # xmin and xmax aren't really the x bounds,
        # because of the odd way grob works.

        # They're just set to the standard x +- width/2 so that clusters
        # can be dodged like other geoms.
        # After position code is rewritten,
        # each dot should have its own box.
        ...
        # Unlike with y above,
        # don't change x because it will cause problems with dodging
      }
      df
    }
  }
